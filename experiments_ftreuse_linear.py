...
from experiments_ft import doc_inputs_all_for_linear
...
@click.option('--plots_prefix', default="ftreuse-linear")
def run_keras_rendered_experiment_cls_extract_types(sqlite_source,
    ...
    def build_model(n_siz, use_only_annotated_wordboxes):
        ...
        inputs_doc, feats_doc = doc_inputs_all_for_linear("", neighbours=neighbours,
                                                     shapes_inp=shapes_inp)
        inputs_nearest, feats_nearest = doc_inputs_all_for_linear("nearest-", neighbours=neighbours,
                                                             shapes_inp=shapes_inp)
        ...
        annotation_all = annotation_all_orig
        ...
        max_pooled_nearest = GlobalMaxPool1D()(annotation_all)

        def timedistributed_concat(packed):
            x, pic = packed
            return K.concatenate([x, K.repeat(pic, K.shape(x)[-2])], axis=-1)

        all = Lambda(timedistributed_concat)([fork_node, max_pooled_nearest])

        # all = Concatenate(axis=-1)([fork_node, repeated])
